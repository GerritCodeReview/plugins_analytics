...
class HashTagsExtractorSpec extends FlatSpec with Matchers with GerritTestDaemon {
  ...
  def hashTagExtractor = HashTagsExtractor(fileRepositoryName, testFileRepository.getRepository, GerritTestDaemon.getInstance(classOf[ChangeNotesCache]))
  ...
  it should "extract hashtags when they are defined in a change" in {
    ...
    hashTagExtractor.tagsOfCommit(commit) should contain only (hashTag1, hashTag2)
  }

  it should "extract empty hashTags when change note has no hashTags defined" in {
    val commit = commitOfChange(newChange())
    hashTagExtractor.tagsOfCommit(commit) shouldBe empty
  }
