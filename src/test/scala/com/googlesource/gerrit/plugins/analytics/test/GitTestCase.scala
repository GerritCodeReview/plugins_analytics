...
trait GitTestCase extends BeforeAndAfterEach {
  ...
  private def threadSpecificDirectoryPrefix = "git-test-case-" + UUID.randomUUID().toString
  ...
  /**
    ...
    *
    */
  protected def branch(repo: GitRepository, name: String, start: String): Ref = {
    repo.git.branchCreate.setName(name).setStartPoint(start).call
    checkout(name, repo)
  }
  ...
  def gitClone(repo: GitRepository): GitRepository = {
    val git = Git.cloneRepository
      .setDirectory(new File(s"$TmpDir/${threadSpecificDirectoryPrefix}_clone"))
      .setURI(repo.getRepository.getDirectory.toURI.toString)
      .call()

    GitUtil.newTestRepository(git.getRepository.asInstanceOf[FileRepository])
  }
  ...
  protected def mergeCommit(committer: String, fileName: String, content: String, repo: GitRepository = testRepo): MergeResult = {
    val currentBranch = repo.getRepository.getBranch
    val tmpBranch = branch(repo, "tmp", currentBranch)
    try {
      commit(committer, fileName, content, repo = repo, branch = tmpBranch.getName)
      checkout(currentBranch, repo = repo)
      mergeBranch(tmpBranch.getName, withCommit = true, repo = repo)
    } finally {
      deleteBranch(repo, tmpBranch.getName)
    }
  }
}
