...
import com.googlesource.gerrit.plugins.analytics.test.{BinaryFilesNoCache, GitTestCase}
...
class NonBinaryFileFilterSpec extends FlatSpec with Matchers with GitTestCase {
  ...
  it should "include regular files" in {
    ...
    NonBinaryFileFilter(new Project.NameKey("foo"), BinaryFilesNoCache).include(walk) shouldBe true
  }
  ...
  it should "not include binary files" in {
    ...
    NonBinaryFileFilter(new Project.NameKey("foo"), BinaryFilesNoCache).include(walk) shouldBe false
  }
}