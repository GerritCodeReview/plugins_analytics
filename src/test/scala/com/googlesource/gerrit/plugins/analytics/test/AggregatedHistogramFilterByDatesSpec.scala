...
@UseLocalDisk
class AggregatedHistogramFilterByDatesSpec extends FlatSpec with GerritDaemonTest with BeforeAndAfterEach with Matchers {

  "Author history filter" should
    "select one commit without intervals restriction" in {

    testRepo.commitFile("file.txt", "some content")
  ...
  it should "select only the second of two commits based on the start timestamp" in {
    val firstCommit = testRepo.commitFile("file.txt", "some content")
    ...
    val secondCommit = testRepo.commitFile("file.txt", "other content", author = person, committer = person)
  ...
  it should "select only the first of two commits based on the end timestamp" in {
    ...
    val firstCommitTs = testRepo.commitFile("file.txt", "some content", author = person, committer = person)
      .getCommitterIdent.getWhen.getTime
    val secondCommitTs = testRepo.commitFile("file.txt", "other content", committer = new PersonIdent(committer, new Date(firstCommitTs + 1000L)))
      .getCommitterIdent.getWhen.getTime
  ...
  it should "select only one middle commit out of three based on interval from/to timestamp" in {
    val firstCommitTs = testRepo.commitFile("file.txt", "some content")
      .getCommitterIdent.getWhen.getTime
    val person = newPersonIdent("Middle person", "middle@company.com", new Date(firstCommitTs + 1000L))
    val middleCommitTs = testRepo.commitFile("file.txt", "other content", author = person, committer = person)
      .getCommitterIdent.getWhen.getTime
    val lastCommitTs = testRepo.commitFile("file.text", "yet other content", committer = new PersonIdent(committer, new Date(middleCommitTs + 1000L)))
      .getCommitterIdent.getWhen.getTime
  ...
  it should "aggregate commits of the same user separately when they are in different branches and branchesExtractor is set" in {
    testRepo.commitFile("file1.txt", "add file1.txt to master branch")

    val clonedRepo = testRepo.gitClone
    clonedRepo.branch("another/branch", "master")
    clonedRepo.commitFile("file2.txt", "add file2.txt to another/branch", branch = "another/branch")
    clonedRepo.push
