...
import com.googlesource.gerrit.plugins.analytics.common.{CommitsStatistics, GsonFormatter, Statistics}
...
class CommitStatisticsSpec extends FlatSpec with GitTestCase with Matchers with Inside {
  ...
  it should "sum to another compatible CommitStatistics generating an aggregated stat" in {
    val commit1 = CommitInfo("sha_1", 1000l, false, Array("file1"))
    val commit2 = CommitInfo("sha_2", 2000l, false, Array("file1"))
    val commit3 = CommitInfo("sha_3", 3000l, false, Array("file2"))
    val commit4 = CommitInfo("sha_4", 1000l, false, Array("file1"))
