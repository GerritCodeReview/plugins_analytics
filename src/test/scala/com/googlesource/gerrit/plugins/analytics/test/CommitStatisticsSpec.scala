...
class CommitStatisticsSpec extends FlatSpec with GerritTestDaemon with TestCommitStatisticsNoCache with Matchers with Inside {
  ...
  it should "sum to another compatible CommitStatistics generating an aggregated stat" in {
    val commit1 = CommitInfo("sha_1", 1000l, false, botLike = false, Set("file1"), hashTags = Set.empty)
    val commit2 = CommitInfo("sha_2", 2000l, false, botLike = false, Set("file1"), hashTags = Set.empty)
    val commit3 = CommitInfo("sha_3", 3000l, false, botLike = false, Set("file2"), hashTags = Set.empty)
    val commit4 = CommitInfo("sha_4", 1000l, false, botLike = false, Set("file1"), hashTags = Set.empty)

    val stat1 = CommitsStatistics(3, 4, false, false, List(commit1, commit2), hashTags = Set.empty)
    val stat2 = CommitsStatistics(5, 7, false, false, List(commit3, commit4), hashTags = Set.empty)

    (stat1 + stat2) shouldBe CommitsStatistics(8, 11, false, false, List(commit1, commit2, commit3, commit4), hashTags = Set.empty)
  }
