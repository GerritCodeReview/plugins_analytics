...
trait GerritDaemonTest extends BeforeAndAfterEach {
  self: Suite =>
  ...
  var testRepo: GitRepository = null

  implicit var testRepository: FileRepository = null
  ...
  override def beforeEach {
    testRepository = daemonTest.getRepository(
      daemonTest.newProject(testSpecificRepositoryName))

    testRepo = GitUtil.newTestRepository(testRepository)
  }
