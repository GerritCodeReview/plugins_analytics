...
class CommitStatisticsCommentLinkSpec extends FlatSpec with GitTestCase with Matchers with Inside {
  ...
  def createCommentLinkInfo(
                             pattern: String, link: Option[String] = None, html: Option[String] = None) = {
  ...
  class TestEnvironment {
    ...
    val commentLinks = Arrays.asList(
      createCommentLinkInfo("(bug\\s+#?)(\\d+)", link = Some("http://bugs" +
        ".example.com/show_bug.cgi?id=$2")),
      createCommentLinkInfo("([Bb]ug:\\s+)(\\d+)", html = Some("$1<a href=\"http://trak.example.com/$2\">$2</a>")))
  ...
  it should "collect simple bugzilla comments" in new TestEnvironment {
    ...
    inside(stats.forCommits(simpleComment)) {
      case List(s: CommitsStatistics) =>
        ...
        s.issues should contain (IssueInfo("bug #23","http://bugs.example" +
          ".com/show_bug.cgi?id=23")
        )
    }
  ...
  it should "collect simple track link" in new TestEnvironment {
    ...
    inside(stats.forCommits(simpleTrackComment)) {
      case List(s: CommitsStatistics) =>
        s.issues should have size 1
        s.issues should contain (IssueInfo("Bug: 1234","Bug: <a " +
          "href=\"http://trak.example.com/1234\">1234</a>"))
    }
    ...
    inside(stats.forCommits(multipleComments)) {
      case List(s: CommitsStatistics) =>
        s.issues should contain allOf(
          ...
          IssueInfo("Bug: 23","Bug: <a href=\"http://trak.example" +
            ".com/23\">23</a>")
        )
    }
