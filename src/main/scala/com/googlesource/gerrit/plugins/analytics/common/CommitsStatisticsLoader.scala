...
import com.googlesource.gerrit.plugins.analytics.{
  AnalyticsConfig,
  CommitInfo,
  IssueInfo
}
...
class CommitsStatisticsLoader @Inject() (
    ...
    val commentInfoList: Seq[CommentLinkInfo] =
      if (config.isExtractIssues)
        projectCache
          ...
          .asScala
          .toList
          .flatMap(_.getCommentLinks.asScala)
      else Seq.empty
    val replacers = commentInfoList.map(info =>
      Replacer(info.`match`.r, Option(info.link).getOrElse(info.html))
    )
        ...
        val oldTree = {
          ...
          if (commit.getParentCount == 0)
            ...
            new CanonicalTreeParser(
              null,
              reader,
              rw.parseCommit(commit.getParent(0).getId).getTree
            )
        }
  ...
  private def extractIssues(
      commitMessage: String,
      replacers: Seq[Replacer]
  ): Seq[IssueInfo] =
