...
object UserActivitySummary {
  ...
  Array[CommitInfo] = {
    val array = (ids, times, merges).zipped.map((id, time, merge) => CommitInfo(id.name, time, merge))
    array
  }
}

case class UserActivitySummary(name: String, email: String, numCommits: Int, commits:
Array[CommitInfo], lastCommitDate: Long)