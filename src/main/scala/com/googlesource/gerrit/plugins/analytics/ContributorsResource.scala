...
(
  ...
  def apply(projectRes: ProjectResource): Response[BinaryResult] = {
    Response.ok(new JsonStreamedResult[UserActivitySummary]
    (getUsersActivities(projectRes)))
  }


  def getUsersActivities(projectRes: ProjectResource): List[UserActivitySummary] = {
    val repo: Repository = repoManager.openRepository(projectRes.getNameKey)
    try {
      userSummary.getCommitters(repo)
    } finally {
