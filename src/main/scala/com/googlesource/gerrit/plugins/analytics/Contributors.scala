...
class ContributorsService @Inject()(repoManager: GitRepositoryManager,
  ...
  def get(projectRes: ProjectResource, startDate: Option[Long], stopDate: Option[Long],
    ...
    ManagedResource.use(repoManager.openRepository(projectRes.getNameKey)) { repo =>
      val commitsHandler = new Statistics(repo)
      histogram.get(repo, new AggregatedHistogramFilterByDates(startDate, stopDate,
        aggregationStrategy))
        .par
        .map(UserActivitySummary.apply(commitsHandler)).toStream
    }
  }
}
...
object UserActivitySummary {
  def apply(commitsHandler: Statistics)(uca: AggregatedUserCommitActivity): UserActivitySummary = {
    val INCLUDESEMPTY = -1
    ...
    uca.key.split("/", INCLUDESEMPTY) match {
      case a@Array(email, year, month, day, hour) =>
        ...
        val stats = commitsHandler.find(uca.getIds.toSeq)
        UserActivitySummary(year, month, day, hour, uca.getName, uca.getEmail, uca.getCount, stats.numFiles,stats.addedLines,stats.deletedLines,
