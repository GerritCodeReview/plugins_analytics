...
class ContributorsCommand @Inject() (
  ...
  @ArgOption(
    ...
    usage =
      "(included) begin timestamp. Must be in the format 2006-01-02[ 15:04:05[.890][ -0700]]"
  )
  ...
  @ArgOption(
    ...
    usage =
      "(excluded) end timestamp. Must be in the format 2006-01-02[ 15:04:05[.890][ -0700]]"
  )
...
class ContributorsResource @Inject() (
  ...
  def setBeginDate(date: String) {
    try {
      ...
      case e: Exception =>
        throw new BadRequestException(s"Invalid begin date ${e.getMessage}")
    }
  }
  ...
  def setEndDate(date: String) {
    try {
      ...
      case e: Exception =>
        throw new BadRequestException(s"Invalid end date ${e.getMessage}")
    }
  }
  ...
  def setGranularity(value: String) {
    try {
      ...
      case e: Exception =>
        throw new BadRequestException(s"Invalid granularity ${e.getMessage}")
    }
  }
...
class ContributorsService @Inject() (
  ...
  def get(
    ...
    ManagedResource.use(repoManager.openRepository(projectRes.getNameKey)) {
      repo =>
        val stats =
          new Statistics(projectRes.getNameKey, commitsStatisticsCache)
        val branchesExtractor =
          extractBranches.option(new BranchesExtractor(repo))
