...
class ContributorsCommand @Inject()(val executor: ContributorsService,
  ...
  @ArgOption(name = "--filter-branch", aliases = Array("-f"),
    usage = "Filter results only for a specific branch", required = false)
  private var filteredBranch: String = null
  ...
  override protected def run =
    gsonFmt.format(executor.get(projectRes, beginDate, endDate,
      granularity.getOrElse(AggregationStrategy.EMAIL), extractBranches, Option(filteredBranch)), stdout)
    ...
    Response.ok(
      new GsonStreamedResult[UserActivitySummary](gson,
        executor.get(projectRes, beginDate, endDate,
          granularity.getOrElse(AggregationStrategy.EMAIL), extractBranches, Option(filteredBranch))))
}
...
class ContributorsService @Inject()(repoManager: GitRepositoryManager,
  ...
  def get(projectRes: ProjectResource, startDate: Option[Long], stopDate: Option[Long],
          aggregationStrategy: AggregationStrategy, extractBranches: Boolean, filteredBranch: Option[String])
  : TraversableOnce[UserActivitySummary] = {
    ...
    ManagedResource.use(repoManager.openRepository(projectRes.getNameKey)) { repo =>
      ...
      val branchesExtractor = (extractBranches || filteredBranch.isDefined).option( new BranchesExtractor(repo, filteredBranch))
