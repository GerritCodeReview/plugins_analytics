...
case class UserActivitySummary(year: String, month: String, day: String, hour: String, name: String, email: String, numCommits: Int,
...
object UserActivitySummary {
  def apply(uca: AggregatedUserCommitActivity): UserActivitySummary = {
    ...
    def get(x: String) = if (x.isEmpty) null else x

    uca.key.split("/", INCLUDESEMPTY) match {
      case a@Array(email, year, month, day, hour) =>
        UserActivitySummary(get(year), get(month), get(day), get(hour), uca.getName, uca.getEmail, uca.getCount,
