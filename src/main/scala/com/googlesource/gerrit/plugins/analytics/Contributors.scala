...
class ContributorsService @Inject()(repoManager: GitRepositoryManager,
  ...
  : TraversableOnce[UserActivitySummary] = {
    ...
    val hashTagExtractor = extractHashTags.option(HashTagsExtractor(changeFinder))
    ...
    ManagedResource.use(repoManager.openRepository(projectRes.getNameKey)) { repo =>
      ...
      histogram.get(repo, new AggregatedHistogramFilterByDates(startDate, stopDate, branchesExtractor, hashTagExtractor, aggregationStrategy))
        .par
