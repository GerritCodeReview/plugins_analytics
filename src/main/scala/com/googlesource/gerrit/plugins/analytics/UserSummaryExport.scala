...
@Singleton class UserSummaryExport {
  def getCommitters(repo: Repository): List[UserActivitySummary] = {
    ...
    authorActivity.par.map(UserActivitySummary.fromUserActivity).toList
  }
}