...
trait HashTagsExtractor {
  def tagsOfCommit(repo: Repository, projectKey: Project.NameKey, revCommit: RevCommit): Set[String]
}

final case class HashTagsExtractorImpl @Inject() (changeNotesCache: ChangeNotesCache) extends HashTagsExtractor {
  private val log = LoggerFactory.getLogger(classOf[HashTagsExtractor])

  // TODO: this is COMPUTATIONALLY INSANE, find a better way to do this
  def objectsToRefs(repo: Repository): collection.Map[AnyObjectId, Set[Ref]] = {
    use(new Git(repo)) {
  ...
  // TODO: this is COMPUTATIONALLY INSANE, find a better way to do this
  def refsToObjects(repo: Repository): Map[String, AnyObjectId] =
    objectsToRefs(repo).foldLeft(Map[String, AnyObjectId]()) { case (acc, (objectId, refs)) =>
      acc ++ refs.toSeq.flatMap { ref =>
        Map(ref.getName -> objectId)
      }.toMap
    }

  override def tagsOfCommit(repo: Repository, projectKey: Project.NameKey, revCommit: RevCommit): Set[String] = {
    val maybeRefs = objectsToRefs(repo).get(revCommit.toObjectId)
    maybeRefs.map { refs =>
      ...
      refs.collectFirst {
        case c if c.getName.startsWith("refs/changes/") => {
          ...
          refsToObjects(repo).get(metaRefString).map { metaRefObjectId =>
